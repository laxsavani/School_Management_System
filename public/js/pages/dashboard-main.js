...
function floatchart() {
  ...
  if (keyCode >= 48 && keyCode <= 57 || keyCode === 8) {
    // Check if the input length is already 10 and prevent further input
...
var phoneNumberInputs = document.getElementsByClassName('phoneNumberInput');
var validationMessage = document.getElementById('validationMessage');

// Loop through all input elements and add event listeners
for (var i = 0; i < phoneNumberInputs.length; i++) {
  phoneNumberInputs[i].addEventListener('keypress', validate);
  phoneNumberInputs[i].addEventListener('input', validate);
}
